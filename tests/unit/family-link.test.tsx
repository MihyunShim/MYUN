...
it('정규화한 코드로 한 번만 연결하고 완료 확인 후 현황을 연다', async () => {
  ...
  const form = screen.getByText('연결하기').closest('form')!;
  fireEvent.submit(form); fireEvent.submit(form);
  expect(mocks.rpc).toHaveBeenCalledTimes(1);
  expect(mocks.rpc).toHaveBeenCalledWith('link_with_invite_code', { code: 'ABC123', rel: '어머니' });
  await act(async () => finish({ error: null }));
  expect(mocks.refresh).not.toHaveBeenCalled();
  fireEvent.click(screen.getByText('가족 현황 보기'));
...
it('잘못된 코드 실패 후 입력을 유지하고 다시 연결한다', async () => {
  ...
  fireEvent.click(screen.getByText('연결하기'));
  await screen.findByText('가족과 연결했어요');
});
