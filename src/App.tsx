...
function Router() {
  const { loading, error, session, profile, onboarded, privacyReady, refresh } = useAuth();
  ...
  if (error) return <Screen><Title>정보를 불러오지 못했어요</Title><ErrorBox message={error} /><BigButton onClick={refresh}>다시 시도</BigButton>{session && <SignOutButton />}</Screen>;
  ...
  if (profile.role === 'A1') {
    return onboarded ? <A1Shell key={profile.id} /> : <Onboarding />;
  }
  return onboarded ? <A2Shell key={profile.id} /> : <OnboardingA2 />;
}
