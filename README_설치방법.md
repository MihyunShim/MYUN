...
이번 진단 화면이 포함된 버전에서는 아래 순서로 원인을 구분합니다.

1. **설정 → 앱 안내 → 설치 버전**을 확인합니다. 표시되는 짧은 코드가 설치한 Git 커밋입니다. 값이 없으면 이전 앱일 수 있습니다.
2. **설정 → 음성 안내 → 목소리 미리 듣기**를 누릅니다. 이 버튼은 알림에 사용할 실제 음성 파일을 재생합니다. 미리 듣기는 미디어 음량을 사용하므로 잠금 화면 알림이 들린다는 보장은 아닙니다.
3. 준비 실패/무음 파일이면 **음성 다시 준비하기**를 누릅니다. 새 파일 검사가 끝날 때까지 기존 정상 파일을 유지합니다.
4. **알림 다시 적용 → 10초 후 시험 알림 → 화면 잠금**으로 확인합니다.
5. 미리 듣기는 되는데 알림은 기본음이라면 설치 버전·음성 선택·준비 상태·잠금 여부를 기록합니다. 실제 예약에서의 iOS 음성 재생은 별도 검수가 필요합니다.

현재 기본 알림 수신은 사용자가 확인했지만, 한국어 음성 알림 수신 성공은 아직 확인되지 않았습니다. 재설치나 앱 데이터 삭제를 첫 해결 방법으로 사용하지 않습니다.

제작 시기는 네 자리 연도와 월 목록으로 입력합니다. 잘못된 값·미래 날짜를 저장하지 않고, 편집을 시작하면 이전 '저장됨' 표시를 지웁니다. 진행률은 현재 켜진 관리 항목과 최근 90일 기록을 기준으로 표시합니다.
