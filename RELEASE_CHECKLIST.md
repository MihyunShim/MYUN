> 개인정보 업데이트(008)를 적용하기 전에 [PRIVACY_IMPLEMENTATION.md](PRIVACY_IMPLEMENTATION.md)의 운영 사실 확정·방침 공개·서버/앱 동시 반영 순서를 완료하세요. 미게시 안내로 가입하지 못하도록 제한합니다.

# 틀니케어 — 검수와 출시 준비

이번 브랜치는 아이폰 개발 설치와 기능 검수를 위한 변경입니다. 이 작업 환경에서는 운영 DB 적용, Apple 계정 서명, TestFlight 업로드, App Store 제출을 실행하지 않았습니다. 사용자가 9월 16일 직접 적용한 004~006과 익명 인증 설정 완료 보고는 별도로 기록합니다. 007·008 SQL 및 실제 개인정보 안내 게시 여부는 아직 적용 확인 전입니다. 현재 앱은 008과 게시 안내가 필요하므로 [개인정보 적용 순서](PRIVACY_IMPLEMENTATION.md)를 먼저 확인합니다.

최종 판단은 기능별 통과 근거로 합니다. 높은 평가 점수나 아래 항목의 구현만으로 출시를 승인하지 않습니다. 결과가 없는 항목은 미검증으로 남기며 실제 기기·실제 시험 서버·사용자 관찰을 자동 테스트로 대체하지 않습니다.

9월 23일 추가 변경·설치 순서: [휴대폰 없이 진행한 개선](PHONE_FREE_IMPROVEMENTS.md). 스토어 소개 초안: [소개 문구](STORE_LISTING_DRAFT.md). 출시 설정 검사: `npm run release:check`.
...
변경 보고: [기기 없이 진행한 안정성·가입 개선](PHONE_FREE_REVIEW_2026-09-27.md).

- [ ] 느린 연결에서 로딩이 끝나고 재조회 안내가 나오는지 확인. 응답 소실 시 저장 실패로 단정하지 않음.
- [ ] 임시 보호자의 홈·설정 로그아웃에 확인창이 나오며 취소하면 계속 이용 가능.
- [ ] 실제 가입 이메일의 재전송·메일 확인·앱 로그인. 자동 검사는 API 모의 응답이며 실제 메일 발송과 구분.
- [ ] 개인정보 문서를 새 생성 명령으로 준비하고 `release:check`로 앱 URL·문서 일치 확인.
